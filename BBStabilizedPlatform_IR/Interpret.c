...
//Pitch
float A_FAST_VER  = 0.060f;
float B_FAST_VER  = 0.040f;
#else
float A_HOR  = 0.382f;
float B_HOR  = 0.049f;
...
#endif
float g_fmaximum_ver_rate = 0.100f*23.0f+0.061f;
float g_fmaximum_hor_rate = 0.382f*23.0f+0.049f;
...
char PointCoordinateMode(unsigned char* msg)
{
	if(GetDMCmode() == ONLINE_STATE)
	{
		...
		{					
			...
			SetDMCflag(DMC_EXE_MODE,0);
			putDMCmode(SET_LABEL_COMMAND);
	...
	if(GetDMCmode() == ONLINE_STATE)
	{
		if(!GetDMCflag(DMC_STOW) || (!GetDMCflag(DMC_EXE_MODE)) )
		{
			...
			SetDMCflag(DMC_PILOT_WINDOW | DMC_RATE | DMC_POINT_2_COORD | DMC_CAM_GUIDE | DMC_PTC_CG,0); // signal for control center		
 			...
 			strcpy(LabelCmdMsg[0].Text,"SAFE");
			LabelCmdMsg[0].Text[strlen(LabelCmdMsg[0].Text)] = 0;										
			Set_LABEL_Visibility(OSD_MODE_ID,1);
	...
	//int id = 0;
		...
		switch(cmd)
		{
			...
			case 0x0:
					...
					if( GetDMCflag(DMC_OSD_ON) || (!GetDMCflag(DMC_EXE_MODE)) )
					{
...
char CenterMode(unsigned char* msg)
{
	if(GetDMCmode() == ONLINE_STATE)
	{
		if(!GetDMCflag(DMC_CENTER_MODE) || (!GetDMCflag(DMC_EXE_MODE)) )
		{
				...
				if(cmd)
				{
					...
					{						
					  ...
					  if(GetDMCmode() == ONLINE_STATE)
					  {
						if(GetDMCflag(DMC_ZOOM_X4))
						{
							g_fmaximum_ver_rate = A_FAST_VER*18.0f+B_FAST_VER;
							g_fmaximum_hor_rate = A_HOR*18.0f+B_HOR;						
							ctrl.state[TIME_ZOOM2] = Z2_SENT_MESSAGE;																							
							ctrl.time[TIME_ZOOM] = GetSysTickCount();							
							putDMCmode(UPDATE_ZOOMX2);					// ZOOM X2		
						}
						else
						if(GetDMCflag(DMC_ZOOM_X2))
						{
							g_fmaximum_ver_rate = A_FAST_VER*23.0f+B_FAST_VER;
							g_fmaximum_hor_rate = A_HOR*37.0f+B_HOR;
 					...
 					if ((WORD)(GetSysTickCount() - ctrl.time[TIME_ZOOM]) >= (WORD)1000) //-- every 3 sec
					{
						...
						{						
							...
							if(GetDMCflag(DMC_ZOOM_X2) )
							{
								...
								g_fmaximum_ver_rate = A_FAST_VER*14.0f+B_FAST_VER;
								g_fmaximum_hor_rate = A_HOR*14.0f+B_HOR;								
								ctrl.state[TIME_ZOOM4] = Z4_SENT_MESSAGE;																																
								ctrl.time[TIME_ZOOM] = GetSysTickCount();								
								putDMCmode(UPDATE_ZOOMX4);					// ZOOM X4
							}
						}													
					}
		...
		switch(i)
		{
			...
			case 0x6:
				...
				if(GetDMCmode() == ONLINE_STATE)
				{
					if(cmd)
					{				
						//if ((WORD)(GetSysTickCount() - ctrl.time[WHITE_BLACK]) >= (WORD)2000) //-- every 1 sec
						//{

								if((!GetDMCflag(DMC_BLACK_HOT)) || (!GetDMCflag(DMC_EXE_MODE)) )	
								{							
					...
					else
					{
						...
						//{
								...
								if((!GetDMCflag(DMC_WHITE_HOT)) || (!GetDMCflag(DMC_EXE_MODE)) )		
								{						
									...
									SetDMCflag(DMC_EXE_MODE,0);
									ctrl.time[WHITE_BLACK] = GetSysTickCount(); 	
									ctrl.state[TIME_WH] = 0;
									putDMCmode(WHITE_HOT);	
								}
						//}
