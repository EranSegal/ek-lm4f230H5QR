...
float k_i_pitch_0 = 0.2f;
float k_i_pitch_1 = 0.1f;


float g_fmindutycycle[2][4] = { {0,10.0f,-10.0f,0}, //PITCH_MOTOR=0
...
// do roll axis
		...
		y_rate_roll = ((ITG3200values[Y_AXIS] - Y_AT_REST) * (FULL_SCALE_RANGE / 32768.0f))*cosf(pfData[10]) - ((ITG3200values[Z_AXIS] - Z_AT_REST) * (FULL_SCALE_RANGE / 32768.0f))*sinf(pfData[10]);		
	    //y_rate_roll = ((ITG3200values[Y_AXIS] - Y_AT_REST) * (FULL_SCALE_RANGE / 32768.0f)) - ((ITG3200values[Z_AXIS] - Z_AT_REST) * (FULL_SCALE_RANGE / 32768.0f));		
...
// do roll axis
  ...
  { 
	...
	#endif
	{
		#ifdef NOTCH_TEST_PITCH
	  	// notch filter
	  	upitch[0] = (float)ITG3200values[X_AXIS];
	  	opitch[0] = (N1 * upitch[0]) + (N2 * upitch[1]) + (N3 * upitch[2]) - (D2 * opitch[1]) - (D3 * opitch[2]); 
	  	ITG3200values[X_AXIS] = (signed short)opitch[0] ;
	    ...
	    y_rate_pitch = ((ITG3200values[X_AXIS] - X_AT_REST)*(FULL_SCALE_RANGE/32768.0f));
		//y_rate_pitch = ((ITG3200values[X_AXIS] - X_AT_REST) * (FULL_SCALE_RANGE / 32768.0f))*cosf(Thetac) - ((ITG3200values[Z_AXIS] - Z_AT_REST) * (FULL_SCALE_RANGE / 32768.0f))*sinf(Thetac);		
