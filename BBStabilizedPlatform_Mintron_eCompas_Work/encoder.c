...
void EncoderInitReset(unsigned long ulBase,enum MOTOR motor,enum A3906Logic logic)
{
	  ...
	  GeneralPitchRoll(motor,logic,motor == PITCH_MOTOR ? ((ulPeriod*25)/100): ((ulPeriod*25)/100));	  
	  while(encChanged)
	  {
	  ...
	  if(ulBase == QEI0_BASE) // PITCH_MOTOR
	  {
		  if(logic == A3906_FORWARD)
		  {
			  ...
			  Motor[motor].max_position = QEIPositionGet(QEI0_BASE)-1000;
		  ...
		  else
		  {
			  ...
			  Motor[motor].min_position = QEIPositionGet(QEI0_BASE)+1000; // 1000;			  
			  //Motor[motor].max_position = 110000; 
	  ...
	  else					//	ROLL_MOTOR
	  {
		  if(logic == A3906_FORWARD)
		  {
			  QEIPositionSet(ulBase, 332800);
			  Motor[motor].min_position = QEIPositionGet(QEI1_BASE)-1000; // 315500;			  
		  ...
		  else
		  {
			  ...
			  Motor[motor].max_position = QEIPositionGet(QEI1_BASE)+1000;
...
QEI0IntHandler(void)
{
    ...
    {
        g_ulEncoderClocks = (SYSCLK_80MHZ - ulNow) + g_ulEncoderPrevious;		
...
EncoderDegPositionGet(unsigned long ulBase)
{
    ...
    //
    return( QEIPositionGet(ulBase)/ 924.444f);	
}
