...
void EncoderInitReset(unsigned long ulBase,enum MOTOR motor,enum A3906Logic logic)
{
	  ...
	  GeneralPitchRoll(motor,logic,motor == PITCH_MOTOR ? ((ulPeriod*30)/100): ((ulPeriod*30)/100));	  
	  while(encChanged)
	  {
	  ...
	  else					//	ROLL_MOTOR
	  {
		  if(logic == A3906_FORWARD)
		  {
			  ...
			  QEIPositionSet(ulBase, 0);
			  Motor[motor].min_position = 20000;			  
			  Motor[motor].max_position = 180000;		
...
void EncoderWhile2Point(enum MOTOR motor,enum A3906Logic driction,unsigned long point)
{
  ...
  if(ulpos > point)
	  GeneralPitchRoll(motor,A3906_REVERSE,20);
  else		
  if(ulpos < point)	
	  GeneralPitchRoll(motor,A3906_FORWARD,20);
...
EncoderTick(unsigned long ulBase)
{
	if(ulBase == QEI0_BASE)
	{
		...
		Motor[PITCH_MOTOR].aangle = Motor[PITCH_MOTOR].position / 924.444f/2.0f;
		...
		{
			if(Motor[PITCH_MOTOR].position == Motor[PITCH_MOTOR].stowpoint)   
			{
	...
	if(ulBase == QEI1_BASE)
	{
		...
		Motor[ROLL_MOTOR].aangle= Motor[ROLL_MOTOR].position / 924.444f/2.0f;	
		...
		{
			if(Motor[ROLL_MOTOR].position == Motor[ROLL_MOTOR].stowpoint)	  
			{
